#!/usr/bin/env python3
"""
Optimized ATC Training Demo - Simple Speed Improvements

This script demonstrates how to achieve 5-10x speedup using existing libraries
while keeping the code simple and maintainable.

Key optimizations:
1. Parallel environments (SubprocVecEnv)
2. Vectorized conflict detection (numpy broadcasting)
3. CUDA acceleration (automatic via Stable-Baselines3)
"""
...
print("=" * 60)
print("Optimized ATC Training Demo")
print("=" * 60)
...
class OptimizedATCEnv(gym.Env):
    """Optimized ATC environment with vectorized operations."""
    ...
    def step(self, action):
        # Handle both single action and vectorized actions
        if isinstance(action, np.ndarray) and action.ndim == 1:
            aircraft_id, heading_change_idx = action
        else:
            # For vectorized environments, action might be a list
            aircraft_id, heading_change_idx = action[0], action[1]
...
def make_env():
    ...
    return OptimizedATCEnv(max_aircraft=5, max_steps=100)
...
def benchmark_parallel_environments():
    ...
    # Single environment
    single_env = DummyVecEnv([lambda: OptimizedATCEnv(max_aircraft=5, max_steps=50)])
    
    # Parallel environments
    num_envs = min(4, mp.cpu_count())
    parallel_env = SubprocVecEnv([make_env for _ in range(num_envs)])
    
    # Benchmark single environment
    start_time = time.time()
    obs = single_env.reset()
    for _ in range(1000):
        action = [single_env.action_space.sample() for _ in range(1)]
